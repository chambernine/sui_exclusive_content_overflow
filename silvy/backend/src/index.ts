...
app.use(
  ...
  cors({
    origin: (origin) => {
      ...
      if (
        origin === "https://sui-exclusive-content-overflow.vercel.app" ||
...
app.patch("/my-album/publish", async (c) => {
  try {
    ...
    const userDocRef = firestoreDb.collection("users").doc(draftAlbum.owner);
    ...
    const existingPublished = userDocSnap.get("published") || [];
    const updatedPublished = [...new Set([...existingPublished, albumAccessId])];
    ...
    console.error("🔥 Error publishing album:", JSON.stringify(error));
    return c.json({
      status: 500,
      error: "Internal Server Error",
    });
...
app.patch("/my-album/publish/:albumId/:blobId", async (c) => {
  try {
    ...
    const publishedAlbum: PublishedAlbum = {
...
app.get("/my-album/purchase/:owner", async (c) => {
  try {
    ...
    const purchased: string[] = userDoc.data()?.purchased || [];
    if (purchased.length === 0) {
      return c.json({ status: 200, data: [] });
    }

    // Step 2: Batch load album metadata
    const albumRefs = purchased.map((albumId) =>
      firestoreDb.collection("albums").doc(albumId)
    );
...
app.get("/my-album/published/:owner", async (c) => {
  try {
    const { owner } = c.req.param();

    // 1. Get the user document
    const userDocRef = firestoreDb.collection("users").doc(owner);
    const userDocSnap = await userDocRef.get();
    ...
    const publishedAlbumIds: string[] = userDocSnap.get("published") || [];

    if (publishedAlbumIds.length === 0) {
      return c.json({ status: 200, data: [] });
    }

    const albumRefs = publishedAlbumIds.map((id) =>
